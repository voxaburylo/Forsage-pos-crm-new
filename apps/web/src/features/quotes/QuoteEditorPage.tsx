...
function DraftItemRow({
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white">
      {/* Desktop view: Основна стрічка позиції */}
      <div className="hidden md:flex items-center gap-2 px-3 py-2.5 bg-white">
      ...
      {/* Mobile view: Основна стрічка позиції */}
      <div className="flex md:hidden flex-col p-4 bg-white gap-3 relative">
        <div className="flex justify-between items-center gap-2">
          <span className="text-xs font-bold text-gray-500">Позиція #{idx + 1}</span>
          <button onClick={() => onRemove(idx)} className="text-gray-400 hover:text-red-500 p-1">
            <Trash2 size={16} />
          </button>
        </div>
        <div>
          <label className="block text-[11px] font-semibold text-gray-400 uppercase mb-1">Назва деталі *</label>
          <input
            placeholder="Назва деталі..."
            value={item.name}
            onChange={(e) => update({ name: e.target.value })}
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="block text-[11px] font-semibold text-gray-400 uppercase mb-1">Артикул</label>
            <input
              placeholder="Арт."
              value={item.sku}
              onChange={(e) => update({ sku: e.target.value })}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-yellow-300"
            />
          </div>
          <div>
            <label className="block text-[11px] font-semibold text-gray-400 uppercase mb-1">К-сть</label>
            <input
              type="number" min="1"
              value={item.qty}
              onChange={(e) => update({ qty: e.target.value })}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-center focus:outline-none focus:ring-2 focus:ring-yellow-300"
            />
          </div>
        </div>
        <div className="flex justify-between items-center gap-2 pt-2 border-t border-gray-100 flex-wrap">
          <div className="flex items-center gap-2">
            <label className="block text-[11px] font-semibold text-gray-400 uppercase">Ціна:</label>
            {item.is_draft_note ? (
              <button
                onClick={() => update({ is_draft_note: false })}
              ...
              <div className="flex items-center gap-1">
                <input
                  ...
                  value={item.sell_price}
                  onChange={(e) => update({ sell_price: e.target.value })}
                  className="w-24 border border-gray-200 rounded-lg px-2 py-1 text-sm text-right focus:outline-none focus:ring-2 focus:ring-yellow-300 bg-white"
                />
                <span className="text-xs text-gray-400">₴</span>
              </div>
            )}
          ...
          >
            {item.variants.length > 0 ? `${item.variants.length} вар.` : '+ Варіанти'}
            {item.expanded ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
          </button>
        </div>
        {bestPrice > 0 && item.variants.length > 0 && (
          <div className="text-xs font-semibold text-green-600 text-right mt-1">
            Найкраща ціна: від {formatMoney(Math.round(bestPrice * 100))}
          </div>
        )}
      </div>
      ...
      {item.expanded && (
        <div className="bg-gray-50 border-t border-gray-200 px-4 py-3 space-y-3">
          <div className="flex items-center justify-between mb-1">
            ...
            <button
              onClick={addVariant}
              className="text-xs text-yellow-600 hover:text-yellow-700 font-semibold flex items-center gap-1"
            >
          ...
          {item.variants.map((v, vi) => (
            <div key={vi} className="space-y-2">
              ...
              {/* Mobile variants */}
              <div className="flex md:hidden flex-col gap-2.5 bg-white rounded-xl p-3 border border-gray-200 relative shadow-sm">
                <div className="flex justify-between items-center gap-2">
                  <button
                    onClick={() => toggleRecommended(vi)}
                    className={`flex items-center gap-1 text-xs font-semibold ${v.is_recommended ? 'text-yellow-500' : 'text-gray-400'}`}
                  >
                    <Star size={14} fill={v.is_recommended ? 'currentColor' : 'none'} />
                    {v.is_recommended ? 'Рекомендовано' : 'Зробити реком.'}
                  </button>
                  <button onClick={() => removeVariant(vi)} className="text-gray-400 hover:text-red-500 p-1">
                    <Trash2 size={14} />
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-400 uppercase mb-0.5">Виробник</label>
                    <input
                      placeholder="Brembo, TRW..."
                      value={v.brand}
                      onChange={(e) => updateVariant(vi, { brand: e.target.value })}
                  ...
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-400 uppercase mb-0.5">Ціна (грн)</label>
                    <input
                      type="number" min="0" step="0.01"
                      placeholder="Ціна"
                      value={v.price}
                      onChange={(e) => updateVariant(vi, { price: e.target.value })}
                      className="w-full border border-gray-200 rounded-lg px-2.5 py-1.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-yellow-300"
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] font-semibold text-gray-400 uppercase mb-0.5">Примітка</label>
                  <input
                    placeholder="Примітка..."
                    value={v.notes}
                    onChange={(e) => updateVariant(vi, { notes: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-yellow-300"
                  />
                </div>
              </div>
            </div>
