...
export default function StaffPage() {
  ...
  const [editForm, setEditForm] = useState({role:'cashier' as UserRole,is_active:true,full_name:'',phone:'',base_rate:'',salaryMode:'only_rate' as SalaryMode,pct_from_revenue:'',pct_from_profit:''})
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      const [usersRes,summaryRes,paymentsRes,rulesRes] = await Promise.all([adminApi.listUsers(),api.get<{data:EmployeeSummary[]}>(`/api/v1/salary/summary?period=${period}`),api.get<{data:SalaryPayment[]}>(`/api/v1/salary?period=${period}`),commissionApi.listRules()])
      setUsers(usersRes.data); setSummary(summaryRes.data??[]); setPayments(paymentsRes.data??[]); setRules(rulesRes.data??[])
  ...
  async function handleCreate(e:React.FormEvent){e.preventDefault();setSaving(true);try{await adminApi.createUser({...addForm,base_rate:addForm.base_rate?Math.round(parseFloat(addForm.base_rate)*100):0});toast.success('Співробітника додано');setAddOpen(false);setAddForm({phone:'',password:'',full_name:'',role:'cashier',base_rate:''});loadData()}catch(err){toast.error(err instanceof Error?err.message:'Помилка створення')}finally{setSaving(false)}}
  ...
  function openModal(u:AdminUser){
    ...
    setEditForm({role:u.role as UserRole,is_active:u.is_active,full_name:u.full_name,phone:u.phone||'',base_rate:u.base_rate?(u.base_rate/100).toString():'',salaryMode:sm,pct_from_revenue:mr?mr.pct_from_revenue.toString():'',pct_from_profit:mr?mr.pct_from_profit.toString():''})
  ...
  async function handleSaveAll(e:React.FormEvent){
    ...
    try {
      ...
      await adminApi.updateUser(selectedUser.id,{role:editForm.role,is_active:editForm.is_active,full_name:editForm.full_name,base_rate:rate,phone:editForm.phone||undefined})
      ...
      setSelectedUser({...selectedUser,role:editForm.role,is_active:editForm.is_active,full_name:editForm.full_name,phone:editForm.phone||selectedUser.phone,base_rate:rate})
      loadData()
  ...
  async function handleAddTransaction(){if(!selectedUser)return;const amount=Math.round(parseFloat(actionForm.amount||'0')*100);if(amount<=0){toast.error('Вкажіть коректну суму');return};setSaving(true);try{await api.post('/api/v1/salary',{employee_id:selectedUser.id,employee_name:selectedUser.full_name||selectedUser.email,amount,type:actionForm.type,method:actionForm.method,period:actionForm.period||period,note:actionForm.note||null});toast.success('Операцію збережено');setActionForm({...actionForm,amount:'',note:''});loadData()}catch(err){toast.error(err instanceof Error?err.message:'Помилка')}finally{setSaving(false)}}
  ...
  const columns = [
    ...
    { key:'base_rate' as const, header:'Ставка', render:(u:AdminUser)=>(<span className="text-sm font-medium text-gray-700">{u.base_rate?formatMoney(u.base_rate):'—'}</span>) },
    ...
    <Layout title="Команда та ЗП">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-5">
                    <div className="bg-gradient-to-br from-amber-50/80 to-yellow-50/40 rounded-xl p-4 space-y-4 border border-amber-100/60">
                      ...
                      {editForm.salaryMode!=='only_pct'&&<Input label="Ставка (грн/місяць)" type="number" min="0" step="0.01" value={editForm.base_rate} onChange={(e)=>setEditForm({...editForm,base_rate:e.target.value})} placeholder="наприклад: 15000"/>}
                      {editForm.salaryMode!=='only_rate'&&(
                        <div className="space-y-3 bg-white/60 rounded-lg p-3 border border-amber-100/50">
                          <p className="text-xs font-semibold text-amber-700">Відсотки від продажів</p>
                          ...
                          <p className="text-[10px] text-gray-400">Відсоток нараховується від особистих продажів менеджера автоматично</p>
                        </div>
                      )}
      ...
      <Modal open={addOpen} onClose={()=>setAddOpen(false)} title="Додати співробітника" size="sm">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <Input label="Ставка (грн/місяць)" type="number" min="0" step="0.01" value={addForm.base_rate} onChange={(e)=>setAddForm(f=>({...f,base_rate:e.target.value}))} placeholder="наприклад: 15000"/>
