...
export default function POSPage() {
  ...
  async function handleConfirmPayment(
    ...
    async function saveOfflineSale() {
      // На касі власна локальна база: чек, покладений у браузерну чергу, не
      // потрапив би на сервер ніколи і не показався б навіть у лічильнику.
      // Повторити оплату безпечно: локальний продаж ідемпотентний за номером
      // операції, тому другий раз створить не дубль, а той самий чек.
      if (desktopBridge()) {
        toast.error('Чек не зберігся на касі. Натисніть «Оплатити» ще раз — дубль не створиться')
        return null
      }
      const scopeKey = session?.user?.id ?? ''
      if (!scopeKey) {
        toast.error('Сеанс касира втрачено — офлайн-чек нікуди зберегти. Увійдіть у програму заново')
        return null
      }
