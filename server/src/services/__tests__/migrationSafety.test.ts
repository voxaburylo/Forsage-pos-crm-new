...
describe('critical migration safety', () => {
  ...
  it('locks financial ledgers and records cash returns in the selected shift', () => {
    const sql = migration('20260729123704_stage2_financial_integrity.sql')
    expect(sql).toContain('CREATE OR REPLACE FUNCTION public.process_return_v3')
    expect(sql).toContain('INSERT INTO public.cash_operations')
    expect(sql).toContain('p_shift_id')
    expect(sql).toContain('REVOKE ALL ON TABLE')
    expect(sql).toContain('GRANT ALL ON TABLE')
    expect(sql).toContain('client_operation_id')
    expect(sql).toContain('sales_payment_amounts_match')
  })
})