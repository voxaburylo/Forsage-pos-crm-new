...
describe('sync role policy', () => {
  ...
  it('lets every role that can push a product also push the brand and category it points at', () => {
    ...
    for (const role of ['owner', 'admin', 'manager', 'storekeeper', 'cashier']) {
      expect(isSyncOperationAllowed(role, 'product.upsert')).toBe(true)
      expect(isSyncOperationAllowed(role, 'brand.upsert')).toBe(true)
      expect(isSyncOperationAllowed(role, 'category.upsert')).toBe(true)
    }
    for (const role of ['tire_worker', 'sto_viewer']) {
      expect(isSyncOperationAllowed(role, 'brand.upsert')).toBe(false)
      expect(isSyncOperationAllowed(role, 'category.upsert')).toBe(false)
    }
    // Renaming or removing a shared reference stays with the owner.
    expect(isSyncOperationAllowed('cashier', 'brand.deleted')).toBe(false)
    expect(isSyncOperationAllowed('cashier', 'category.deleted')).toBe(false)
  })
