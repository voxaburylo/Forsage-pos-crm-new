...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (!parsed.success) throw new AppError('VALIDATION_ERROR', 'Невірні дані повернення', 422, parsed.error.flatten())
    const ret = await returnService.createReturn(req.user!.id, parsed.data)
