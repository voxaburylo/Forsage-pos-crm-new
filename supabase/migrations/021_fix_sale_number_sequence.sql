-- 021_fix_sale_number_sequence.sql
-- Виправлення race condition в process_sale:
--   COUNT(*) + 1 → nextval('sale_number_seq')
-- Додаємо аудит_log та shop_settings RLS (перенесено з 022)

-- ============================================================
-- 1. Створюємо SEQUENCE для sale_number
-- ============================================================
CREATE SEQUENCE IF NOT EXISTS sale_number_seq START 1;

-- Ініціалізуємо з поточного макс значення
SELECT setval('sale_number_seq', COALESCE((SELECT COUNT(*) FROM sales), 0), true);

-- ============================================================
-- 2. Оновлюємо process_sale — використовуємо nextval замість COUNT(*)
-- ============================================================
...
BEGIN
    ...
    -- ✅ Беремо номер із SEQUENCE замість COUNT(*) — захист від race condition
    ...
    INSERT INTO sales (tenant_id,sale_number,customer_id,cashier_id,shift_id,status,subtotal,discount,total,payment_method,is_debt,notes)
    VALUES (p_tenant_id,v_sale_number,p_customer_id,p_cashier_id,p_shift_id,'completed',v_subtotal,p_discount,v_total,p_payment_method,p_payment_method='debt',p_notes)
