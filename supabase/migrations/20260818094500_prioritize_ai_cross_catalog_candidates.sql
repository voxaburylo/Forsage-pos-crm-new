...
BEGIN
  ...
  WITH candidates AS (
    ...
    ORDER BY
      CASE
        WHEN p.sku ~* '^AUTO-'
          AND p.name ~* '([[:alpha:]][[:alnum:]./_-]*[0-9]|[0-9][[:alnum:]./_-]*[[:alpha:]])'
          THEN 0
        WHEN NULLIF(BTRIM(p.oem_number), '') IS NOT NULL
          OR NULLIF(BTRIM(p.normalized_supplier_article), '') IS NOT NULL
          OR p.sku !~* '^AUTO-'
          THEN 1
        WHEN p.name ~* '([[:alpha:]][[:alnum:]./_-]*[0-9]|[0-9][[:alnum:]./_-]*[[:alpha:]])'
          THEN 2
        ELSE 3
      END,
      COALESCE(state.updated_at, p.created_at),
      p.id
    LIMIT v_batch_size
